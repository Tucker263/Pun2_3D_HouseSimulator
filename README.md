...
---
## 苦労した課題(所有権によるリアルタイム同期、途中参加の整合性、データ管理の一貫性、通信料の削減)
...
3画面エミュレータによる動作：
- リアルタイム同期(室内照明、街灯日照、内外装表面)
- 3Dオブジェクト操作(3人で動かす)
- 途中参加->途中退出->途中参加の同期(3人で動かす)
- セーブ／ロード(複数フォルダの作成、一人だけ)
- ゲストの強制切断処理(セーブデータを一つにするため)、3画面で
- ゲストのルーム参加制限(3人制限版)/他ルーム参加可能、4画面で
- オンライン/オフラインモード(ルーム名同じに設定)、2画面で
